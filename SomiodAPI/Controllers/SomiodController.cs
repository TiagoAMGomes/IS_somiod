...
namespace SomiodAPI.Controllers
{
	...
	public class SomiodController : ApiController
    {
		...
		{
			string selectQueryString = $"SELECT * FROM containers WHERE name = '{containerName}' " +
		...
		public IHttpActionResult Post(string appName, [FromBody] XElement xmlInput)
		{
			try
			{
				...
				using (SqlConnection connection = new SqlConnection(connStr))
				{
					string insertQueryString = $"INSERT INTO containers (name, parent) " +
						$"VALUES ('{containerName}', (SELECT id FROM applications WHERE name = '{appName}'))";
					string selectQueryString = $"SELECT * FROM containers WHERE name = '{containerName}' " +
						$"AND parent = (SELECT id FROM applications WHERE name = '{appName}')";
		...
		public IHttpActionResult Put(string appName, string containerName, [FromBody] XElement xmlInput)
		{
			try
			{
				...
				using (SqlConnection connection = new SqlConnection(connStr))
				{
					string updateQueryString = $"UPDATE containers SET name = '{newContainerName}' WHERE name = '{containerName}' " +
						$"AND parent = (SELECT id FROM applications WHERE name = '{appName}')";
					string selectQueryString = $"SELECT * FROM containers WHERE name = '{newContainerName}' " +
						$"AND parent = (SELECT id FROM applications WHERE name = '{appName}')";
				...
				using (SqlConnection connection = new SqlConnection(connStr))
				{
					...
					try
					{
						...
						if (reader.Read())
						{
							...
							string deleteQueryString = $"DELETE FROM containers WHERE name = '{containerName}' " +
								$"AND parent = (SELECT id FROM applications WHERE name = '{appName}')";
